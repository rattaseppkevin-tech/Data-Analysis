...
oil_array = np.array(oil_prices)

oil_series = pd.Series(oil_array, name="oil_prices", dtype="float")
...
mean_float = oil_series.values.mean()
print(f"Mean (float): {mean_float}")

mean_int = oil_series.astype("int").mean()
print(f"Mean (int): {mean_int}")